$(document).ready(function() {
    ...
    $('.page-title').html('<h4 class="title">TASK LIST </h4>');
    $('.sub_actions').html('<div class="dropdown"><button class="btn btn-border btn-md" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Actions</button><div class="dropdown-menu" aria-labelledby="dropdownMenuButton"><ul><li><a class="dropdown-item" href="#" id="import_task">Import</a></li><li><a class="dropdown-item" id="task_export" href="#">Task Export</a></li><li><a class="dropdown-item"  id="task_bulk_update" href="#">Bulk update</a></li><li><a class="dropdown-item" id="task_email" href="#">Status Email</a></li></ul></div></div>');

    $('body').on('click', '#card_click', function(event) {
        event.preventDefault();
        $('.page-title').html('<h4 class="title">CARD LIST</h4>');
    });
    $('body').on('click', '#row_click', function(event) {
        event.preventDefault();
        $('.page-title').html('<h4 class="title">TASK LIST</h4>');
    });
    $('body').on('click', '#import_task', function(event) {
        event.preventDefault();
        window.location = import_task_url;
    });
    ...
    $('body').on('click', '#reset_task_filter', function(event) {
        ...
        $('#ticket_task_type_id').val(-1);
        $('#ticket_task_type_id').trigger('change');

        $('#tasks_type_id').val(-1);
        $('#tasks_type_id').trigger('change');
